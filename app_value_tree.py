...
st.set_page_config(page_title="Value Tree — Visual v4 (fix)", layout="wide")
...
def fmt_value(val, unit):
    ...
    try:
        if abs(float(val) - int(float(val))) < 1e-9:
            return f"{int(float(val)):,} {unit}".strip()
    except Exception:
        pass
...
def label_multiplicative(df, env, u, v):
    ...
    if m:
        ...
        try:
            valf = float(val)
            if 0 <= valf <= 1:
                return f"{rid}: {valf*100:.1f}%"
            return f"{rid}: {valf:,.3f}"
        except:
            return ""
    return ""
...
def cards_dot(df: pd.DataFrame, env: Dict[str,float], mode_label: str) -> str:
    nodes, edges = build_graph(df)
    depth = depth_by_topology(df)
    dot = []
    dot.append('digraph G {')
    dot.append('rankdir=RL;')  # root à esquerda
    ...
    # Ordenação manual por depth/group/order_num com arestas invisíveis
